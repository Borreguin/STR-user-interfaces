...
import {createNewBahia, deleteBahia, editBahia} from "../../../../Common/FetchData/V2SRFetchData";
...
export function BahiaAdministration(props) {
    ...
    const handleEdit = (event) => {
        ...
        editBahia(selectedInstalationId, bahia.document_id, bahiaForm).then((response) => {
            console.error(response)
            setLoading(false);
            // requestReload();
        });
    ...
    return (
        <Form noValidate validated={validated}>
            ...
            {!bahia &&
                <Button className="col-4" variant="primary" type="button" disabled={loading} onClick={handleCreate}>
                    Crear Bahia
                </Button>}
            {bahia &&
                <Row>
                    <Button className="col-4" variant="primary" type="button" disabled={loading} onClick={handleEdit}>
                        Actualizar Bahia
                    </Button>
                    <br className="col"/>
                    <Button className="col-4" variant="danger" type="button" disabled={loading} onClick={handleRemove}>
                        Eliminar Bahia
                    </Button>
                </Row>}
        </Form>
