...
class FilterV2SRNodes extends Component<V2SRNodeFilterProps, V2SRNodeFilterState> {
  selected: {};
  selected_id: {};
  installations: any[];
  url: string;
  ...
  _handle_filter_change = () => {
    ...
    for (const idx in this.state.nodes) {
    ...
    for (var idz in this.installations) {
      let utr = this.installations[idz];
      if (
        utr.utr_tipo === this.selected["utr_tipo"] &&
        utr.instalacion_nombre === this.selected["instalacion_nombre"]
      ) {
  ...
  componentDidMount = () => {
    ...
    this._update_filter().then(() =>{
      this.setState({ loading: false });
    });
  };

  _update_filter = async () => {
    let path = SRM_API_URL + "/admin-sRemoto/v2/nodos/";
    await fetch(path)
      .then((res) => res.json())
      .then((json) => {
        if (!json.success) {
          ...
          console.log("_update_filter nodes", nodes);
  ...
  _utr_type = () => {
    ...
    for (var idx in node.entidades) {
      ...
      if (
        ...
        this.installations = entidad.utrs;
        if (this.installations === undefined || this.installations.length === 0) {
          // No hay UTRs que mostrar:
          options["instalacion_tipo"] = [
            <option key={1}>Presione aquí</option>,
          ];
          options["instalacion_nombre"] = [];
          this.selected["instalacion_tipo"] = undefined;
          this.selected["instalacion_nombre"] = undefined;
        } else {
          options["instalacion_tipo"] = this._filter_options(this.installations, "instalacion_tipo");

          if (options["instalacion_tipo"].length > 0) {
            this.selected["instalacion_tipo"] = options["instalacion_tipo"][0].props.children;
          } else {
            this.selected["instalacion_tipo"] = undefined;
          }
        }
  ...
  _utr_names = () => {
    ...
    let options = this.state.options;
    this.installations.sort((a, b) => (a.utr_nombre > b.utr_nombre ? 1 : -1));
    options["utr_nombre"] = this._filter_options(
      this.installations,
      "utr_nombre",
      "instalacion_tipo",
      this.selected["instalacion_tipo"]
    );
...
export default FilterV2SRNodes;
